...
def insert_data(conn, cursor, query, tuple):
    try:
...
def get_airbnb_data(hook, execution_date, file_suffix, cols, tablename):
    ...
    result = insert_data(conn, cursor, query, tuples)
...
def get_lga_data(hook, file, cols, tablename):
    start = dt.datetime.now()
...
def get_g01_data(hook, file, cols, tablename):
    start = dt.datetime.now()
...
def get_g02_data(hook, file, cols, tablename):
    start = dt.datetime.now()
